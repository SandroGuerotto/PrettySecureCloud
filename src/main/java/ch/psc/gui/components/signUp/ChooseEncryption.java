...
/**
 * Handles all user data to customize his encryption. The user can choose what level of encryption
 * he wants to use for encrypting his data.
 *
...
public class ChooseEncryption extends VBox implements SignUpFlow {
  ...
  private void initialize() {
    ...
    JFXComboBox<CipherAlgorithms> algorithms =
        createCipherAlgoDropDown(Arrays.stream(CipherAlgorithms.values())
            .filter(CipherAlgorithms::isSupported).collect(Collectors.toList()));
   ...
   */
  private JFXComboBox<CipherAlgorithms> createCipherAlgoDropDown(
      List<CipherAlgorithms> cipherAlgorithms) {
  ...
  /**
   * Invoked by a cipher algorithm dropdown selection. The relevant cipher algorithm value is
   * claimed from the CipherFactory Delegates registration setup to the corresponding method.
   *
