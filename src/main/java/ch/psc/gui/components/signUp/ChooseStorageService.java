...
/**
 * Handles all sign-in process of the supported storage services. Each Service has an individual
 * implementation of their authorization process.
 *
...
public class ChooseStorageService extends VBox implements SignUpFlow {
  ...
  private void initialize() {
    ...
    List<JFXButton> services =
        Arrays.stream(StorageService.values()).filter(StorageService::isSupported)
            .map(this::createStorageButton).collect(Collectors.toList());
  ...
  /**
   * Creates a button for a {@link StorageService}. Adds a graphic to the button.
   *
  ...
  /**
   * Invoked by a storage service button. Delegates registration setup to the corresponding method.
   *
  ...
  /**
   * Creates a popup with instructions to connect with a dropbox account. The user is required to
   * log in and grants permission to access his data. After, a token needs to be copied into an
   * input field. If the token is valid, it gets automatically converted into an access-token and
   * inserted into {@link #chosenServices}. The popup closes.
   */
  private Stage createDropboxPopup(final DropBoxService dropBoxService, final DbxWebAuth auth,
      final DbxWebAuth.Request authRequest) {
    ...
    Hyperlink hyperlink =
        new Hyperlink(String.format(Config.getResourceText("signup.dropbox.link"), authorizeUrl));
    ...
    dialogVbox.getChildren().addAll(hyperlink,
        ...
        new Text(Config.getResourceText("signup.dropbox.step3")), codeInput);
