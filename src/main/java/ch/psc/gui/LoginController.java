...
/**
 * Controller for the LoginView. Contains everything the controller has to reach in the login view
 * and all methods the login view calls based on events.
 *
 * @author bananasprout
 */
...
public class LoginController extends ControlledScreen {
  ...
  private void login() {
    if (enterMailTextfield.validate() && enterPasswordTextfield.validate()) {
      try {
        User user = authenticationService.authenticate(enterMailTextfield.getText(),
