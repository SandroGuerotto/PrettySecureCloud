...
public class FileBrowserController extends ControlledScreen {
  ...
  public FileBrowserController(Stage primaryStage,
  ...
  private void initialize() {
    ...
    service_tab.getSelectionModel().selectedItemProperty()
        .addListener((observable, oldValue, newValue) -> {
          if (newValue == null)
            return;
          activeStorageService =
              storageManager.getStorageOptions().get(service_tab.getTabs().indexOf(newValue));
    ...
    storageManager.getStorageOptions().forEach(fileStorage -> {
      stat_pane.getChildren().add(buildStatPane(fileStorage));
      service_tab.getTabs().add(buildServiceTab(fileStorage));
    });
    activeFileBrowserTreeTableView = (FileBrowserTreeTableView) service_tab.selectionModelProperty()
        .get().getSelectedItem().getContent();
    currentPath.set(activeStorageService.getRoot());
  ...
  private FileBrowserTreeTableView buildTreeView(FileStorage fileStorage) {
    ...
    storageManager.loadManagedFiles(fileStorage, fileStorage.getRoot(),
        fileTree -> buildTree(fileTree, root));
  ...
  /**
   * Handles drag-n-drop over the UI. Retrieves all dragged item and uploads them via
   * {@link StorageManager}.
   *
  ...
  private void handleUploadDragDrop(FileStorage fileStorage, DragEvent event) {
    ...
    if (event.getDragboard().hasFiles()) {
      db.getFiles().forEach(file -> storageManager.uploadFiles(fileStorage, file,
          progress -> uploadProgress(progress, activeStorageService, file)));
    }
  ...
  private void updateNavBar(String path, FileStorage fileStorage, FileBrowserTreeTableView tree) {
    ...
    nav_pane.getChildren()
        .add(createNavButton(fileStorage, fileStorage.getName(), fileStorage.getRoot(), tree));

    StringBuilder pathBuilder =
        new StringBuilder(fileStorage.getRoot() + fileStorage.getSeparator());
  ...
  /**
   * Splits path at the separator into a stream. First entry is skipped, because its root entry.
   *
   * @param path current path
   ...
   */
  private JFXButton createNavButton(FileStorage fileStorage, String name, final String path,
      FileBrowserTreeTableView tree) {
    JFXButton root = new JFXButton(name);
    root.setOnAction(event -> {
      storageManager.loadManagedFiles(fileStorage, path,
          fileTree -> buildTree(fileTree, tree.getRoot()));
  ...
  /**
   * On change of path (eg user selects a folder), the new directory gets loaded via
   * {@link StorageManager} and gets displayed.
   *
   * @param path new path
   * @param fileStorage active file storage
   * @param tree active tree to update content
   */
  private void updateCurrentDirectory(String path, FileStorage fileStorage,
      FileBrowserTreeTableView tree) {
    Platform.runLater(() -> {
  ...
  private void buildTree(List<PscFile> files, TreeItem<FileRow> root) {
    Platform.runLater(() -> {
      ...
      files.forEach(file -> root.getChildren().add(new TreeItem<>(new FileRow(file))));
    });
  ...
  private void download() {
    PscFile file =
        activeFileBrowserTreeTableView.getSelectionModel().getSelectedItem().getValue().getFile();
    storageManager.downloadFiles(activeStorageService, file,
        progress -> downloadProgress(progress, file));
  }
  ...
  /**
   * Seeks and retrieves the progress level with given id. if no label is found a new one gets added
   * to {@link #statusPane}
   *
   * @param id label id
  ...
  private Label getProgressLabel(String id) {
    return (Label) statusPane.getChildren().stream().filter(node -> node.getId().equals(id))
        .findFirst().orElseGet(() -> {
  ...
  /**
   * Builds storage service statistic pane. Shows the amount of free and total space
   *
  ...
  private Region buildStatPane(FileStorage service) {
    ...
    availStorage.progressProperty().addListener(
        (observable, oldValue, newValue) -> applyStatCSS(availStorage, newValue.doubleValue()));
    availStorage.setProgress(
        usedStorageSpace.divide(totalStorageSpace, 3, RoundingMode.HALF_EVEN).doubleValue());

    Label label =
        new Label(storageAmountText(totalStorageSpace.longValue(), usedStorageSpace.longValue()));

    service.getUsedStorageSpaceProperty().addListener((observable, oldValue, newValue) -> {
      availStorage
          .setProgress(newValue.divide(totalStorageSpace, 3, RoundingMode.HALF_EVEN).doubleValue());
      label.setText(storageAmountText(totalStorageSpace.longValue(), newValue.longValue()));
    });

    label.setPadding(new Insets(0, 5, 0, 0));
    stack.getChildren().addAll(availStorage, label);
  ...
  private String storageAmountText(long totalStorageSpace, long usedStorageSpace) {
    return String.format("%s / %s", JavaFxUtils.formatSize(usedStorageSpace),
        JavaFxUtils.formatSize(totalStorageSpace));
  }
  ...
  private void applyStatCSS(ProgressBar progressBar, double used) {
    progressBar.getStyleClass().removeAll("progress-red", "progress-orange", "progress-green",
        "progress-yellow");
