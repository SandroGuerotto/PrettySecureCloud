...
public class DropBoxService implements FileStorage {
  ...
  private final SimpleObjectProperty<BigDecimal> usedStorageSpaceProperty =
  ...
  /**
   * Create a new instance for Dropbox communication. Only used for getting an access token.
   */
  ...
  public static DropBoxService connect(Map<String, String> accountData) {
    ...
    DbxCredential dbxCredential = new DbxCredential(accountData.get("access_token"),
        Long.decode(accountData.get("expires_at")), accountData.get("refresh_token"),
  ...
  public void upload(String fileName, InputStream inputStream) {
    try {
      client.files().uploadBuilder(currentPath + "/" + fileName).withMode(WriteMode.OVERWRITE)
          .withAutorename(true).uploadAndFinish(inputStream);
      ...
      while (true) {
        for (Metadata metadata : result.getEntries()) {
          ...
          if (metadata instanceof FileMetadata fileMetadata) {
            file = new PscFile(metadata.getName(), metadata.getPathLower(), fileMetadata.getSize(),
                fileMetadata.getClientModified(), false);
          } else {
  ...
  /**
   * Creates a new Dropbox configuration. Default parameter: clientID=Pretty-Secure-Cloud,
   * Locale=de_CH
   *
  ...
  /**
   * Reads the dropbox application configuration file. The credentials are need to identify the
   * psc-application to use dropbox services
   *
  ...
  /**
   * Called after the user has visited the authorization URL and copy/pasted the authorization code
   * that Dropbox gave them.
   *
   * @param auth OAuth request configuration
   * @param userCode The authorization code shown to the user when they clicked "Allow" on the
   *        authorization, page on the Dropbox website.
   * @return a Map with OAuth access token used for authorization with Dropbox servers. Use key
   *         "access_token" to get the token.
   * @throws Exception if wrong code was entered
   */
  public Map<String, String> finishFromCode(DbxWebAuth auth, final String userCode)
      throws Exception {
    try {
      DbxAuthFinish authFinish = auth.finishFromCode(userCode.trim());
      return Map.of("access_token", authFinish.getAccessToken(), "refresh_token",
          authFinish.getRefreshToken(), "expires_at", authFinish.getExpiresAt().toString());
    } catch (DbxException e) {
