...
/**
 * Implementation of a basic authentication service. Each user has its own file. Each file is
 * uniquely identifiable through a combination of email and password.
 *
...
public class JSONAuthenticationService implements AuthenticationService {
  ...
  /**
   * Creates a hash with content using a secret key. HmacSHA256 is used to calculate hash. If an
   * error occurs during calculation of the HmacSHA256, a fallback base64-encoded value is used.
   *
  ...
  protected static class JSONUser {
    ...
    JSONUser(String username, String mail, String password,
        Map<StorageService, Map<String, String>> storageServiceConfig,
        Map<String, Map<String, String>> keyChain, String downloadPath) {
    ...
    /**
     * Helper method to serialize {@link User} to a JSON-format. Used to custom serialize data of
     * user.
     *
    ...
    public static JSONUser toJson(User user) throws KeyDeSerializationException {
      return new JSONUser(user.getUsername(), user.getMail(), user.getPassword(),
          user.getStorageServiceConfig(), serializeKeyChain(user.getKeyChain()),
          user.getDownloadPath());
    }

    /**
     * Create a Map of type of key and a string-map with algorithm and secret. Uses
     * {@link #serializeKey(Key)} to create the string-map.
     *
    ...
    private static Key deserializeToKey(Map<String, String> map)
      ...
      if (algorythm.contains("RSA")) {
        ...
        secretKey = new SecretKeySpec(map.get(JSON_TOKEN_SECRET).getBytes(KEY_ENCODING),
            map.get(JSON_TOKEN_ALGORITHM));
      }
     ...
     * @throws KeyDeSerializationException If a Key cannot be deserialized, the cause is wrapped in
     *         this exception.
     */
        ...
        for (Entry<String, Map<String, String>> entry : keyChain.entrySet()) {
          try {
            ...
            throw new KeyDeSerializationException(
                "Could not deserialize Key with name '" + entry.getKey() + "'", e);
          }
        }
      return deserialized;
    }
     ...
     * @throws KeyDeSerializationException If a Key cannot be deserialized, it will be wrapped in
     *         this exception.
     */
    public static User fromJson(JSONUser jsonUser) throws KeyDeSerializationException {
      assert jsonUser != null;
      return new User(jsonUser.username, jsonUser.mail, jsonUser.password,
          jsonUser.storageServiceConfig, deserializeKeyChain(jsonUser.keyChain),
