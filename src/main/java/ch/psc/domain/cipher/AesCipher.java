...
public class AesCipher extends PscCipher {
  ...
  /**
   * This method is a specification for the AES algorithm which makes use of so called
   * initialization vectors (IV). The relevant parameters (nonce) must be randomnized each time the
   * is being encrypted.
   *
   * @param file {@link PscFile}
   * @return AlgorithmParameterSpec - a new one if the file is currently decrypted, and the already
   *         existing one, if the file already encrypted (the used nonce is stored in the
   *         {@link PscFile} and can be obtained via {@link PscFile#getNonce()}
   */
  @Override
